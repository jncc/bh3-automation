-- FUNCTION: public.bh3_remove_duplicate_geometries(name, name, character varying[], character varying[], name, name)

-- DROP FUNCTION public.bh3_remove_duplicate_geometries(name, name, character varying[], character varying[], name, name);

CREATE OR REPLACE FUNCTION public.bh3_remove_duplicate_geometries(
	input_schema name,
	input_table name,
	ranking_columns character varying[],
	output_columns character varying[] DEFAULT NULL::character varying[],
	output_schema name DEFAULT NULL::name,
	output_table name DEFAULT NULL::name,
	OUT success boolean,
...
BEGIN
	BEGIN
		...
		RAISE INFO 'bh3_remove_duplicate_geometries: input table %: %', in_table, (clock_timestamp() - start_time);
		start_time := clock_timestamp();
		...
		IF output_table IS NULL THEN
			out_table := 'tmp_remove_duplicate_geometries';
			sqlstmt := format('CREATE TEMPORARY TABLE %1$I AS %2$s', 
							  out_table, sqlstmt);
		ELSE
			IF output_schema IS NOT NULL THEN
				out_table := quote_ident(output_schema) || '.' || quote_ident(output_table);
			ELSE
				out_table := quote_ident(output_table);
			END IF;
			sqlstmt := format('CREATE TABLE %1$s AS %2$s', 
							  out_table, sqlstmt);
		END IF;

		RAISE INFO 'bh3_remove_duplicate_geometries: executing SQL statement %: %', sqlstmt, (clock_timestamp() - start_time);
		start_time := clock_timestamp();

		EXECUTE sqlstmt;
		
		RAISE INFO 'bh3_remove_duplicate_geometries: executed SQL statement: %', (clock_timestamp() - start_time);
		start_time := clock_timestamp();

		IF output_table IS NULL THEN
			/* empty input table */
			EXECUTE format('DELETE FROM %1$s', in_table);
			
			/* insert rows from temporary into input table */
			EXECUTE format('INSERT INTO %1$s '
						   ...
						   'SELECT %2$I'
							   ...
							   ',%4$s '
						   'FROM %5$s',
						   in_table,pk_col,geom_col,target_list,out_table);
			GET DIAGNOSTICS rows_affected = ROW_COUNT;
			CALL bh3_drop_temp_table(out_table);
			
			RAISE INFO 'bh3_remove_duplicate_geometries: replaced % input table rows: %', 
				rows_affected, 
				(clock_timestamp() - start_time);
		ELSE
			/* add  primary key constraint and spatial index to output_table */
			...
			RAISE INFO 'bh3_remove_duplicate_geometries: added primary key constraint and spatial index to %.%: %', 
				QUOTE_IDENT(output_schema), 
...
ALTER FUNCTION public.bh3_remove_duplicate_geometries(name, name, character varying[], character varying[], name, name)
    OWNER TO postgres;
